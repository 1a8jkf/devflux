...
export const FileSystemService = {
  ...
  async createProject(name: string, type: 'html' | 'node' | 'react' = 'html', dependencies: string[] = []) {
    ...
    // Check for duplicates without adding giant timestamps
    if (!IS_WEB) {
      let counter = 2;
      while ((await getInfoAsync(projectPath)).exists) {
        projectPath = `${PROJECTS_ROOT}${id}-${counter}/`;
        counter++;
      }
      if (counter > 2) id = `${id}-${counter - 1}`;
    }
