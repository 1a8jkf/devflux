...
export default function SQLTerminalScreen() {
  ...
  const updateTabById = (tabId: string, updates: Partial<QueryTab>) => {
  ...
  const handleRunQuery = async () => {
    ...
    setIsRunning(true);
    updateTabById(targetTabId, { isRunning: true, errorMsg: '' });
    try {
      ...
      setIsRunning(false);
      updateTabById(targetTabId, { isRunning: false });
    }
  };
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ alignItems: 'center' }}>
          {tabs.map(tab => (
            ...
            >
              ...
              {tab.isRunning && <ActivityIndicator size="small" color={theme.colors.accentBlue} style={{ marginLeft: 6, marginRight: 4 }} />}
              {tabs.length > 1 && (
